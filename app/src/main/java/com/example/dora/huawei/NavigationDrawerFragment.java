...
public class NavigationDrawerFragment extends Fragment {
    ...
    private static List<NavigationDrawerMenuItem> getNavigationDrawerMenu() {
        ...
        int[] icons = {
            ...
            R.drawable.ic_skill,
            R.drawable.ic_social_menu,
        };

        String[] titles = { "Education", "Work Experience", "Project Showcase", "Technical Skills", "Social"};


        for (int i = 0; i < icons.length && i < titles.length; i++) {
            NavigationDrawerMenuItem item = new NavigationDrawerMenuItem(icons[i], titles[i]);
            menu.add(item);
        }
